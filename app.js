        ...
        result.data.forEach(r => {
            div.innerHTML += `
                <div class="mb-3 border-bottom pb-2">
                    <strong>🍴 ${r.recipe_name || r[0]}</strong> 
                    <span class="badge badge-emergency float-end">清掉 ${r.emergency_count || r[2]} 樣</span>
                    <br><small class="text-muted">${r.description || r[1] || ''}</small>
                </div>
            `;
        });
    } catch (error) {
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="p-3 border rounded bg-white shadow-sm h-100 d-flex flex-column">
                            ...
                            <div class="d-flex justify-content-end mt-2">
                                <button class="btn btn-outline-primary btn-sm me-1" onclick="openRecipeModal(${r.id})">編輯</button>
                                <button class="btn btn-outline-danger btn-sm" onclick="deleteRecipe(${r.id})">刪除</button>
                            </div>
                        </div>
                    </div>
                `;
